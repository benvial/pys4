...
extern "C"
{
	...
	{
		return PyFloat_Check(obj) || PyLong_Check(obj) || PyLong_Check(obj);
	}
...
#define GETSTATE(m) ((struct module_state *)PyModule_GetState(m))
	...
	*/
	...
	/*
	Description: the use is same as structure 'S4Excitation_Data'.
	...
	*/
	...
	static PyObject *S4Sim_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
	{
		...
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "O&n:New", kwlist, &lattice_converter, &(Lr[0]), &nbasis))
		{
			return NULL;
		}
	...
	static PyObject *S4Sim_LoadSolution(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "s:LoadSolution", kwlist, &fname))
		{
			return NULL;
		}
		// printf"Parsed args!\n");
	...
	static PyObject *S4Sim_SaveSolution(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		static char *kwlist[] = {"Filename", NULL};
		const char *fname;
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "s:SaveSolution", kwlist, &fname))
		{
			return NULL;
		}
		// int err;
	...
	static PyObject *S4Sim_SetMaterial(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		Material *M;
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "sO&:SetMaterial", kwlist, &name, &epsilon_converter, &epsdata))
		{
			return NULL;
		}
		...
		else
		{
			 ...
			 * [ g h i ]    [     i ]
			 */
			M->eps.abcde[0] = epsdata.eps[0];
			M->eps.abcde[1] = epsdata.eps[1];
			M->eps.abcde[2] = epsdata.eps[2];
			M->eps.abcde[3] = epsdata.eps[3];
			M->eps.abcde[4] = epsdata.eps[6];
			M->eps.abcde[5] = epsdata.eps[7];
			M->eps.abcde[6] = epsdata.eps[8];
			M->eps.abcde[7] = epsdata.eps[9];
			M->eps.abcde[8] = epsdata.eps[16];
			M->eps.abcde[9] = epsdata.eps[17];
		}
	...
	static PyObject *S4Sim_AddLayer(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		double thickness;
		const char *matname;
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "sds:AddLayer", kwlist, &name, &thickness, &matname))
		{
			return NULL;
		}
	...
	static PyObject *S4Sim_AddLayerCopy(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "sds:AddLayerCopy", kwlist, &name, &thickness, &layername))
		{
			return NULL;
		}
	...
	static PyObject *S4Sim_SetLayerThickness(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		const char *name;
		double thickness;
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "sd:SetLayerThickness", kwlist, &name, &thickness))
		{
			return NULL;
		}
	...
	static PyObject *S4Sim_RemoveLayerRegions(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "s:RemoveLayerRegions", kwlist, &name))
		{
			return NULL;
		}
	...
	static PyObject *S4Sim_SetRegionCircle(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "ss(dd)d:SetRegionCircle", kwlist, &layername, &matname, &center[0], &center[1], &radius))
		{
			return NULL;
		}
	...
	static PyObject *S4Sim_SetRegionEllipse(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "ss(dd)d(dd):SetRegionEllipse", kwlist, &layername, &matname, &center[0], &center[1], &tilt, &halfwidths[0], &halfwidths[1]))
		{
			return NULL;
		}
		layer = Simulation_GetLayerByName(&(self->S), layername, NULL);
	...
	static PyObject *S4Sim_SetRegionRectangle(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "ss(dd)d(dd):SetRegionRectangle", kwlist, &layername, &matname, &center[0], &center[1], &tilt, &halfwidths[0], &halfwidths[1]))
		{
			return NULL;
		}
		layer = Simulation_GetLayerByName(&(self->S), layername, NULL);
	...
	static PyObject *S4Sim_SetRegionPolygon(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "ss(dd)dO&:SetRegionPolygon", kwlist, &layername, &matname, &center[0], &center[1], &tilt, &polygon_converter, &polydata))
		{
			return NULL;
		}
		layer = Simulation_GetLayerByName(&(self->S), layername, NULL);
	...
	static PyObject *S4Sim_SetExcitationPlanewave(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		cp.real = 0;
		cp.imag = 0;
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "(dd)|DDn:SetExcitationPlanewave", kwlist, &angle[0], &angle[1], &cs, &cp, &order))
		{
			return NULL;
		}

		pol_s[0] = sqrt(cs.real * cs.real + cs.imag * cs.imag);
		pol_s[1] = atan2(cs.imag, cs.real);
		pol_p[0] = sqrt(cp.real * cp.real + cp.imag * cp.imag);
		pol_p[1] = atan2(cp.imag, cp.real);
		angle[0] *= (M_PI / 180.);
		angle[1] *= (M_PI / 180.);
	...
	{
		Py_complex f;
		if (!PyArg_ParseTuple(args, "D:SetFrequency", &f))
		{
			return NULL;
		}
	...
	static PyObject *S4Sim_GetEpsilon(S4Sim *self, PyObject *args)
	{
		int ret;
		double r[3], feps[2];
		if (!PyArg_ParseTuple(args, "ddd:GetEpsilon", &r[0], &r[1], &r[2]))
		{
			return NULL;
		}
	...
	static PyObject *S4Sim_OutputLayerPatternPostscript(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "s|s:OutputLayerPatternPostscript", kwlist, &layername, &filename))
		{
			return NULL;
		}
	...
	static PyObject *S4Sim_OutputStructurePOVRay(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "|s:OutputStructurePOVRay", kwlist, &filename))
		{
			return NULL;
		}
	...
	static PyObject *S4Sim_GetPropagationConstants(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "s|d:GetPropagationConstants", kwlist, &layername))
		{
			return NULL;
		}
	...
	static PyObject *S4Sim_GetAmplitudes(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "s|d:GetAmplitudes", kwlist, &layername, &offset))
		{
			return NULL;
		}
	...
	static PyObject *S4Sim_GetPowerFlux(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "s|d:GetPowerFlux", kwlist, &layername, &offset))
		{
			return NULL;
		}
	...
	static PyObject *S4Sim_GetPowerFluxByOrder(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "s|d:GetPowerFluxByOrder", kwlist, &layername, &offset))
		{
			return NULL;
		}
	...
	static PyObject *S4Sim_GetStressTensorIntegral(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "s|d:GetStressTensorIntegral", kwlist, &layername, &offset))
		{
			return NULL;
		}
	...
	static PyObject *S4Sim_GetLayerVolumeIntegral(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "ss:GetLayerVolumeIntegral", kwlist, &layername, &strwhat))
		{
			return NULL;
		}
	...
	static PyObject *S4Sim_GetLayerZIntegral(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "s(dd):GetLayerZIntegral", kwlist, &layername, &r[0], &r[1]))
		{
			return NULL;
		}
	...
	static PyObject *S4Sim_GetFields(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		if (!PyArg_ParseTuple(args, "ddd:GetFields", &r[0], &r[1], &r[2]))
		{
			return NULL;
		}
	...
	static PyObject *S4Sim_GetFieldsOnGrid(S4Sim *self, PyObject *args, PyObject *kwds)
	{
		...
		if (!PyArg_ParseTupleAndKeywords(args, kwds, "d(nn)s|s:GetFieldsOnGrid", kwlist, &z, &nxy[0], &nxy[1], &fmt, &fbasename))
		{
			return NULL;
		}
			...
			for (k = 0; k < 2; ++k)
			{
				...
				for (j = 0; j < nxy[1]; ++j)
				{
					...
					for (i = 0; i < nxy[0]; ++i)
					{
						...
						for (i3 = 0; i3 < 3; ++i3)
						{
							PyTuple_SetItem(pj, i3, PyComplex_FromDoubles(F[k][2 * (3 * (i + j * nxy[0]) + i3) + 0], F[k][2 * (3 * (i + j * nxy[0]) + i3) + 1]));
						}
					}
				}
			}
	...
	static PyObject *S4SpectrumSampler_GetSpectrum(S4SpectrumSampler *self, PyObject *args)
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			PyTuple_SetItem(retObj, i, PyTuple_Pack(2, PyFloat_FromDouble(pt[0]), PyFloat_FromDouble(pt[1])));
		}
		return retObj;
	}
	...
	static PyTypeObject S4Sim_Type = {
		...
		PyVarObject_HEAD_INIT(NULL, 0) "S4.Simulation", /*tp_name*/
	...
	static PyTypeObject S4SpectrumSampler_Type = {
		/* The ob_type field must be initialized in the module init function
		 * to be portable to Windows without using C++. */
		PyVarObject_HEAD_INIT(NULL, 0) "S4.SpectrumSampler", /*tp_name*/
...
#define INITERROR return NULL
	PyMODINIT_FUNC PyInit__S4(void)
	{
		...
		if (PyType_Ready(&S4Sim_Type) < 0)
		{
			INITERROR;
		}
		// if(PyType_Ready(&S4Interpolator_Type) < 0){ INITERROR; }
		if (PyType_Ready(&S4SpectrumSampler_Type) < 0)
		{
			INITERROR;
		}

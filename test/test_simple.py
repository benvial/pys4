...
def test_simple():
    ...
    h = 0.22
    period = [p, p]
    S = S4.New(Lattice=((period[0], 0), (0, period[1])), NumBasis=21)
    ...
    # frequency sweep
    wavelength_space = np.linspace(1.3, 1.7, 3)
    ...
    wavelength_um = float(wavelength_p_nm) / 1000
    # [8.0,14.0]
    freq = 1 / float(wavelength_um)
    S.SetFrequency(freq)  # unit 2*pi*c_const / a
    ...
    os.remove("output_PythonTest.pov")
